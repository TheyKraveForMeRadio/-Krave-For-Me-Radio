...
export default function MixtapeStore({ mixtapes = [] }) {
  ...
  async function handleFile(e) {
    ...
    if (!file) return
    if (!supabase) {
      alert('Supabase not configured. Fill NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in .env.local')
      return
    }
    setUploading(true)
    try {
      ...
      // Create a simple local record; in production save metadata to DB
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:16}}>
        {uploads.map((m,i)=>(
          <div key={i} style={{background:'#111',padding:12,borderRadius:8}}>
            ...
            <p>{m.price}</p>
            {m.preview && (
              <audio controls src={m.preview} style={{width:'100%'}} />
            )}
